-- Migration: Add function to get barracas with computed open status
-- This eliminates the N+1 problem by computing open status in a single query
...
CREATE INDEX IF NOT EXISTS idx_barracas_partnered_name ON barracas(partnered DESC, name ASC);
CREATE INDEX IF NOT EXISTS idx_barracas_location_lower ON barracas(lower(location));
CREATE INDEX IF NOT EXISTS idx_barracas_name_lower ON barracas(lower(name));
CREATE INDEX IF NOT EXISTS idx_barracas_barraca_number_lower ON barracas(lower(barraca_number));

-- Add full-text search index for better text search performance
CREATE INDEX IF NOT EXISTS idx_barracas_fts ON barracas USING gin(to_tsvector('portuguese', name || ' ' || COALESCE(description, '')));
