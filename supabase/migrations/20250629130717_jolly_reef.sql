/*
  # Enhanced Hours Management with UTC Support

  1. New Columns
    - `open_time_utc` (time) - Opening time in UTC
    - `close_time_utc` (time) - Closing time in UTC  
    - `timezone` (text) - Barraca's local timezone
    - `business_hours` (jsonb) - Flexible hours storage

  2. New Tables
    - `business_hours` - Detailed scheduling per day of week
  ...
  3. Functions
    ...
    - `convert_utc_to_local()` - Convert UTC time to local
    - `is_barraca_open_now()` - Real-time open status
    - `get_barraca_hours()` - Get hours in any timezone
    - `update_barraca_hours()` - Easy hours management

  4. Security
    - RLS enabled on business_hours table
...
-- All INSERT statements now have consistent column counts
