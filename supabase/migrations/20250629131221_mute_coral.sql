/*
  # Multi-Environment Database Schema Setup
  
  This migration creates separate schemas for different environments:
  - dev: Development environment
  - qa: Quality Assurance environment  
  - uat: User Acceptance Testing environment
  - prod: Production environment
  
  Each schema contains the complete database structure with appropriate
  data seeding for the environment type.
*/
...
BEGIN
  ...
  IF barraca_count >= 1 THEN
    EXECUTE format('
      ...
      (''barraca-uruguay'', ''Barraca Uruguay'', ''203'', ''Ipanema'', 
       ...
       ''{\"phone\": \"+55 21 99237-1601\", \"email\": \"contato@barracauruguay.com.br\"}'',
  ...
  -- Continue with more barracas based on count...
  ...
  IF schema_name != 'prod' THEN
    EXECUTE format('
      ...
      (''test@%I.com'', ''{\"newBarracas\": true, \"specialOffers\": true}'', NOW() - INTERVAL ''1 week''),
      (''qa@%I.com'', ''{\"newBarracas\": true, \"specialOffers\": false}'', NOW() - INTERVAL ''3 days'')
      ON CONFLICT (email) DO NOTHING', schema_name, schema_name, schema_name);
  END IF;
